...
public class Main {
    ...
    private static int runTest(Test test) {
        ...
        File[] dir = new File("C:\\Users\\margh\\Downloads\\APD-master\\APD-master\\tema2\\checker\\in" + File.separator + test.inFolder).listFiles();
        if (dir != null) {
            for (File input : dir) {
        ...
        try {
            String output = new Scanner(new File("C:\\Users\\margh\\Downloads\\APD-master\\APD-master\\tema2\\checker\\in" + File.separator + test.inFolder + File.separator + "out")).useDelimiter("\\Z").next();
